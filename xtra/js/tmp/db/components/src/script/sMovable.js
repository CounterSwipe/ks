define(function (require) {
  ...
  Crafty.s("SET_MOVABLE", {
    ...
    cellVacant: function (u, cell) {
      ...
      if (c.search(cKey + 3000) === 1) {
        return true;
      } else {
        if (cell > 1) {
          if (c.search(cKey + 4000) === u.keyz.p) {
            return true;
          } else {
            return this.winCollision(u, Crafty(c.search(cKey + 5000)));
          }
        }
        return false;
      }
    ...
    winCollision: function (u, foe) {
      if (u.keyz.spd < foe.keyz.spd) {
        return true;
      } else if (u.keyz.spd > foe.keyz.spd) {
        return false;
      } else {
        if (u.keyz.f > foe.keyz.f) {
          return true;
        } else {
          return false;
        }
