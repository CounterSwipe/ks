define(function() {
  ...
  Crafty.s("SET_ENERGY", {
    init: function() {
      ...
      vued({ x: 20, y: 98, w: 0, h: 1.5 }).color(0, 255, 255, 1);
      Crafty.e("NuVue, WebGL, etop").vued({ x: 0, y: 97.5, w: 100, h: 2.5 });
      this.render(e.energy);
    },
    eMsg: function(eMsg) {
      let eng = Crafty("_ENERGY").get(0);
      eng.energy["p" + eMsg.p] -= eMsg.e;
      if (eMsg.p === 1) { this.render(eng.energy); }
    },
    eUpdate: function() {
      let eng = Crafty("_ENERGY").get(0);
      eng.energy.xe = 2;
