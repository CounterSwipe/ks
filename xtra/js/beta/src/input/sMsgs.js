define(function() {
  ...
  Crafty.c("GetMsgs", {
    ...
    getEid: function() {
      let m = this.getMsgs();
      if (m.msgs.size === m.msgs.max) { return false; }
      else {
        m.msgs.eid++;
        return m.msgs.eid;
      }
    ...
    onMsg: function(msg) {
      let e = this.getEid();
      if (e) { this.mPush({ e: e, m: "deploy", d: msg }); }
    },
    mPush: function(e) {
  ...
  Crafty.s("SET_MSGS", {
    ...
    deploy: function(i) {
      ...
      Crafty.s("SET_ENERGY").eMsg({ p: p, e: energy });
      if (p === 1) {
      ...
      if (d.search(uNum + 1700)) { u.addComponent("_SPLASH"); }
      if (d.search(uNum + 1800)) { u.addComponent("_TOWER"); }
      if (d.search(uNum + 1900)) { u.addComponent("_URGENCY"); }
      this.dAdj(u);
    },
    dAdj: function(u) {
      u.keyz.aX = 6.25;
