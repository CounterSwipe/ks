define({
    "moveTo": {
        "onAdd": function(self) {
            console.log(self);
            var db, make, model, ueid;
            //var moveSpd = self.moveTo[self.mMoveSpd].search(make);//get unit|make moveSpd per hashtable|module (set via deck @start per db)
            var moveSpd = db.actors["3"][make]; //get unit|make moveSpd per db
            self.modules[self.mMoveSpd].add(ueid, moveSpd);
            this.preDraw(self, 0, "js-" + ueid + "pose", "url(/wp-content/uploads/" + "u" + make + "m" + model + "pose" + ".png)");
            this.preDraw(self, 0, "js-" + ueid + "pose", "u" + make + "m" + model + "move");
        },
        "update": function(self) {
            console.log(self);
            var ueid;
            var dur = self.modules[self.mMoveSpd].search(ueid);
            switch (dur) {
                case (0):
                    this.onEnd(self, ueid);
                    break;
                default:
                    this.onTick(self, ueid, dur);
            }
        },
        "onTick": function(self, ueid, dur) {
            console.log(self);
            --dur;
            self.modules[self.mMoveSpd].add(ueid, dur);
            this.preDraw(self, 0, "js-" + ueid + "width", this.getWidth()); //calculate distance % per tick & origin|destination
            this.preDraw(self, 0, "js-" + ueid + "padtop", this.getPad()); //""
            this.preDraw(self, 0, "js-" + ueid + "martop", this.getMartop()); //""
        },
        "onEnd": function(self, ueid) {
            console.log(self);
            var make, model;
            self.modules[self.mMoveSpd].remove(ueid);
            this.preDraw(self, 0, "js-" + ueid + "pose", "u" + make + "m" + model + "move");
            this.preDraw(self, 0, "js-" + ueid + "pose", "url(/wp-content/uploads/" + "u" + make + "m" + model + "pose" + ".png)");
        },
        "draw": function(self, kind, jID, var1, var2) {
            var eid = self.getEID();
