define(function() {
    ...
    mcb: function(r) {
      ...
      if (p) {
        if (p !== r.r.p) {
          ...
          if (cell === r.r.cKey) {
            console.log("dmg:", foe, r.r.dmg);
            f.keyz.hp.now = f.keyz.hp.now - r.r.dmg;
            f.addComponent("_HPDMG");
            let rc = r.clone();
            rc.r = r.r;
            rc.removeComponent("_LAUNCHED").addComponent("EXPLODE");
            console.log(rc, rc.__c);
            if (r.__c["ONIMPACT"]) { this.explode(r); }
          }
          else { return; }
        }
      }
      if (r.r.range <= 0) { this.explode(r); }
    },
    explode: function(r) {
  ...
  Crafty.s("SET_HP", {
    ...
    setHP: function(u) {
      if (u.keyz.hp.now <= 0) { this.exhaust(u); }
      else {
    ...
    exhaust: function(u) {
      ...
      u.destroy();
      console.log("unit exhausted, TODO exhaust unclaim cell, gfx (perhaps @ remove uImg|hp etc) momentDropCheck, callback");
    },
    ...
    render: function(u) {
      ...
      console.log(u[0], u.keyz.hp.now, u.keyz.hp.max, u.keyz.hp.now / u.keyz.hp.max, ((u.keyz.hp.now / u.keyz.hp.max) * 100) >> 0, hPerc);
      if (hPerc > 20) {
        Crafty(u.keyz.hKey).alpha = hPerc;
      }
      else if (hPerc > 10) {
        Crafty(u.keyz.hKey).alpha = .5;
