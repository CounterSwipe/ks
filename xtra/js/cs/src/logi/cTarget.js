  ...
  Crafty.s("SET_TARGET", {
    ...
    seekTarget: function(u) {
      ...
      if (!u.__c["_TARGETING"] && this.setTarget(u)) {
        ...
        if (u.__c["_TOWER"]) { u.removeComponent("_MOVABLE"); }
        this.launch(u);
      }
      else { return; }
    ...
    setTarget: function(u) {
      let dX = u.keyz.dX;
      if (!dX || this.outOfBounds(u)) { return false; }
      else {
        let p = u.keyz.p;
    ...
    launch: function(u) {
      ...
      if (d.search(uNum + 2500)) { r.addComponent("ONIMPACT"); }
      if (u.__c["_URGENCY"]) { atkDur = 25; }
      u.keyz.atkT = delay + atkDur;
      let cKey = u.keyz.dX === 1 ? u.keyz.cKeyR : u.keyz.cKeyL;
      r.rocket({ delay: atkDur, spd: spd, range: range, dmg: dmg, dur: 1 / atkDur, p: p, dX: u.keyz.dX, cKey: cKey, mT: spd }).
