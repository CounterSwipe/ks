define(function() {
  ...
  Crafty.s("SET_CAPTURE", {
    ...
    handle: function(p) {
      ...
      if (m.__c["CAPTURED"]) { this.render(m.search(p), p); }
      else { this.getCapture(p); }
    },
    getCapture: function(p) {
      ...
      for (let i = 0; i < 6; i++) {
        ...
        if (cp) { if (cp === p) { cu.push(c.search(cKey + 5000)); } }
      }
      if (cu.length) { this.setCapture(cu, p); }
      else { return; }
    },
    ...
    getCarry: function(a, b) {
      //console.log(a, b);
      if (a === b || Crafty(a).keyz.f > Crafty(b).keyz.f) { return a; }
      else { return b; }
    },
