define(function() {
  /*global Crafty*/
  Crafty.c("VUE", {
    vue: { w: 0, h: 0, a: 0, b: 0, c: 320, d: 454 },
    //448 //480 //568
  ...
  Crafty.c("bg", {
    setbg: function(grd) {
      let grds = [
        ...
        ["rgba(255,153,0,1)", "rgba(255,255,0,1)"]]; //7:ylo
      let grd1 = grds[grd][0];
      let grd2 = grds[grd][1];
  ...
  Crafty.c("TXT", {
    ...
    //txt: function(a, x, y, w, fF, fS, fA, fC, fT) {
    txt: function(tx) {
      let ff = ['Michroma', 'Orbitron', 'Russo One'];
      let tA = ['left', 'center', 'right'];
      this.fSize = tx.fS;
      this.textFont({ family: ff[tx.fF] }).textAlign(tA[tx.fA]);
      if (tx.fT) { this.textFont({ type: 'italic' }); }
      this.attr({ alpha: tx.a }).color(tx.fC);
      this.vued({ x: tx.x, y: tx.y, w: tx.w });
      this.unselectable();
      this.nuTxt();
      return this;
    },
